...
Rails.application.configure do
  ...
  # Store uploaded files on Cloudflare R2 or local file system (see config/storage.yml for options).
  config.active_storage.service = ENV.fetch('CLOUDFLARE_ENDPOINT', nil).present? ? :cloudflare : :local

  # Serve Active Storage files via redirect (not proxy) for better performance
